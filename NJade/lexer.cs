...
namespace NJade
{
    class Lexer
    {
public static string lex(string str, string filename) {
  ...
  return JSON.parse(JSON.stringify(lexer.getTokens()));
     ...
     * @api private
     */
    public Lexer(string str, string filename, LexerOptions options = null) {
      options = options ?? LexerOptions.Default;
      //Strip any UTF-8 BOM off of the start of `str`, if it exists.
      str = str.StartsWith('\uFEFF'.ToString()) ? str.Substring(1) : str;
